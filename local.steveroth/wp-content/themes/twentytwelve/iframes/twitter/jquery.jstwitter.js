$(function() {		
	...
	JQTWEET = {
	    ...
	    user: 'stevenrothmusic', //username
	    numTweets: 4, //number of tweets
	    appendTo: '#jstwitter',
	    useGridalicious: false,
	    template: '<div class="item"><div class="tweet-wrapper"><span class="text">{TEXT}</span>\
	               <div class="info"><span class="time"><a href="{URL}" target="_blank">{AGO}</a></span>\
	               by <span class="user">{USER}</span></div></div></div>',
	    ...
	    loadTweets: function() {
	        ...
	        $.ajax({
	            url: '/wp-content/themes/twentytwelve/iframes/twitter/grabtweets.php',
	            type: 'POST',
	            dataType: 'json',
	            data: request,
